...
const Conversation = ({isChatActive, setIsChatActive}) => {
    ...
    return (
        <div className={`${isChatActive ? "hidden lg:block" : "lg:w-1/3 w-full h-screen shadow-xl py-5"}`}>
            ...
            {recipent.map((data) => (
                <>
                    <ul key={data?.id}>
                        <li>
                            <Link to={`/chat/${data?.id}`} onClick={() => setIsChatActive(!isChatActive)}>
